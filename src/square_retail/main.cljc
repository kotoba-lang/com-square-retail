(ns square-retail.main
  "Kotodama WASM entrypoint for the square-retail clean-room actor (L4) — Clojure port.
  ...
  facts (`square_retail.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "square_retail")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "squarere_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "squarere_cus"
    :fields [:email :firstName :lastName :phone]                         :required [:email :firstName]
    :coerce {}                                            :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "squarere_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"square_retail.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "square-retail-compat" :tier tier :entities entities} 200])
...
(defn start! [] :square-retail-compat/ready)
